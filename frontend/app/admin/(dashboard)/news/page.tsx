...
interface News { id: number; title: string | null; excerpt: string | null; image: string | null; date: string | null; content: string | null; }

const EMPTY: Omit<News, "id"> = { title: null, excerpt: null, image: null, date: null, content: null };
...
export default function NewsAdmin() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    const { data } = await supabaseAdmin.from("news").select("*").order("id", { ascending: false });
    setNews(data || []);
    setLoading(false);
  }, []);
  ...
  const openEdit = (n: News) => { setForm({ title: n.title, excerpt: n.excerpt, image: n.image, date: n.date, content: n.content }); setModal("edit"); setEditId(n.id); };
  ...
  const save = async () => {
    ...
    let err;
    if (modal === "create") {
      ({ error: err } = await supabaseAdmin.from("news").insert([form]));
    } else {
      ({ error: err } = await supabaseAdmin.from("news").update(form).eq("id", editId!));
    }
    setSaving(false);
    if (err) setAlert({ type: "error", msg: err.message });
    else { setAlert({ type: "success", msg: "Đã lưu bài viết!" }); closeModal(); fetch(); }
          ...
          news.map(n => (
            <div key={n.id} style={{
              ...
              <div style={{ padding: 16 }}>
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 8 }}>{n.date || "—"}</div>
            ...
            <div className="modal-body">
              <div className="form-grid cols-1">
                ...
                <div className="form-group">
                  ...
                  <input className="form-control" value={form.date || ""} onChange={e => f("date", e.target.value)} placeholder="2024-01-15" />
                </div>
                <div className="form-group">
